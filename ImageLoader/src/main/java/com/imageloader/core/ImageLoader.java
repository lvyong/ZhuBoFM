...
import com.imageloader.utils.FileUtils;
...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageView imageView,
		...
		if (bmp != null && !bmp.isRecycled()) {
			 ...
			 * String.format(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE, memoryCacheKey));
			...
			DisplayBitmapTask displayBitmapTask = new DisplayBitmapTask(bmp,
					imageLoadingInfo);
			new Handler().post(displayBitmapTask);
	...
	public void saveImageOnDisc(File targetFile, String uri)
	 ...
	 */
	public void downLoadImage(final String uri) {
		checkExecutors();
		imageLoadingExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					saveImageOnDisc(imageFile, uri);
					configuration.discCache.put(uri, imageFile);
				} catch (IOException e) {
	...
	private void checkExecutors() {
		...
		if (cachedImageLoadingExecutor == null
			...
			cachedImageLoadingExecutor = Executors
					.newSingleThreadExecutor(configuration.displayImageThreadFactory);
		}
	}
	...
	public File getFilePath(String uri) {
		if (configuration != null && configuration.discCache != null
				&& configuration.discCache.get(uri) != null) {
			return configuration.discCache.get(uri);
		}
		return null;
	}
	 ...
	 * 
	 * @param uri
	 * @return true if the image exist in local,otherwise;
	 */
	public boolean isLocalExist(String uri) {
		return configuration.discCache.get(uri).exists();
	}
	 ...
	 * @param uri
	 */
	public void deletePic(String uri) {
